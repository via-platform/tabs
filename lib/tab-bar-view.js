...
module.exports = class TabBarView {
    constructor({manager, pane, location}){
        ...
        this.subscriptions.add(this.pane.onWillDestroyItem(({item}) => this.manager.removeItemFromGroup(item)));
    ...
    onMouseEnter(){
        for(const tab of this.getTabs()){
            ...
            tab.element.style.maxWidth = width.toFixed(8) + 'px';
        }
    }
